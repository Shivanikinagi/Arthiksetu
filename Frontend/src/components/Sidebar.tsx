...
export function Sidebar({ isOpen, onClose, onNavigate, currentPage }: SidebarProps) {
    ...
    return (
        <>
            {/* Backdrop */}
            <div
                className={`absolute inset-0 bg-black/50 z-[60] transition-opacity duration-300 backdrop-blur-sm ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'
                    }`}
                onClick={onClose}
            />
            ...
            {/* Drawer */}
            <div
                className={`absolute top-0 left-0 bottom-0 w-[280px] bg-[#0A1F44] z-[70] transition-transform duration-300 ease-out shadow-2xl flex flex-col ${isOpen ? 'translate-x-0' : '-translate-x-full'
                    }`}
            >
                {/* Header */}
